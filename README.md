...
| <a name="input_default_cleanup_policies"></a> [default\_cleanup\_policies](#input\_default\_cleanup\_policies) | Default cleanup policies to apply to repositories. Used when repository's enable\_default\_cleanup\_policies is true (default). Repository-specific cleanup\_policies are merged with these defaults. | <pre>map(object({<br>    action = string<br>    condition = optional(object({<br>      tag_state             = optional(string)<br>      tag_prefixes          = optional(list(string), [])<br>      version_name_prefixes = optional(list(string), [])<br>      package_name_prefixes = optional(list(string), [])<br>      older_than            = optional(string)<br>      newer_than            = optional(string)<br>    }), {})<br>    most_recent_versions = optional(object({<br>      package_name_prefixes = optional(list(string), [])<br>      keep_count            = optional(number, null)<br>    }), {})<br>  }))</pre> | <pre>{<br>  "keep-protected-tags": {<br>    "action": "KEEP",<br>    "condition": {<br>      "tag_prefixes": [<br>        "latest",<br>        "main",<br>        "master",<br>        "develop",<br>        "stage",<br>        "v1",<br>        "v2",<br>        "v3",<br>        "v4",<br>        "v5",<br>        "v6",<br>        "v7",<br>        "v8",<br>        "v9"<br>      ],<br>      "tag_state": "TAGGED"<br>    }<br>  },<br>  "keep-semantic-versions": {<br>    "action": "KEEP",<br>    "condition": {<br>      "tag_state": "TAGGED",<br>      "version_name_prefixes": [<br>        "0.",<br>        "1.",<br>        "2.",<br>        "3.",<br>        "4.",<br>        "5.",<br>        "6.",<br>        "7.",<br>        "8.",<br>        "9.",<br>        "0-",<br>        "1-",<br>        "2-",<br>        "3-",<br>        "4-",<br>        "5-",<br>        "6-",<br>        "7-",<br>        "8-",<br>        "9-"<br>      ]<br>    }<br>  },<br>  "keep-tagged-images": {<br>    "action": "KEEP",<br>    "most_recent_versions": {<br>      "keep_count": 5<br>    }<br>  },<br>  "remove-old-images": {<br>    "action": "DELETE",<br>    "condition": {<br>      "older_than": "7776000s",<br>      "tag_state": "ANY"<br>    }<br>  }<br>}</pre> | no |
